...
#
# Writes: dist/legendofmana-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix legendofmana \
  --exe-name Legend_of_Mana__Recompiled \
  --display-name "Legend of Mana  Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Legend of Mana disc" \
